...
public async Task<IActionResult> CreatePackagePayment([FromQuery] int packageId)
{
    ...
    var baseUrl = $"{Request.Scheme}://{Request.Host}";
    var returnUrl = $"{baseUrl}/api/paypal/package-success?packageId={packageId}";
    var cancelUrl = $"{baseUrl}/api/paypal/cancel";
...
public IActionResult PackageSuccess([FromQuery] string token, [FromQuery] int packageId)
{
    return Redirect($"http://localhost:5173/payment-success?packageId={packageId}&paymentId={token}");
}
// ============================
